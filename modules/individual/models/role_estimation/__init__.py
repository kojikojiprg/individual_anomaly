...
class RoleEstimation(LightningModule):
    def __init__(
        ...
        self._callbacks = [
            ModelCheckpoint(
                ...
                filename=f"re_{data_type}_seq{self._seq_len}_min",
                monitor="d_loss",
                mode="min",
                save_last=True,
            ),
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        # make ground truth
        adv_real_gt = torch.ones((batch_size,), dtype=torch.float32).to(self.device)
        ...
        # pred generator
        z = torch.randn((batch_size, self._config.model.G.d_z)).to(self.device)
        fake, attn_g = self._G(z)
        adv_fake, aux_fake, f_d_fake, attn_d_fake = self._D(fake)

        if optimizer_idx == 0:
            g_loss = self._l_adv(adv_fake.view(-1), adv_real_gt)
            self.log_dict(
                {
                    "g_loss": g_loss,
                },
